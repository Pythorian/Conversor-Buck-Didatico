...
%                                                                         %
%   v5.0 - Alterada a posição do zero do PI                               %
%   v4.0 - Alterada a frequência de corte do controlador de corrente      %
...
L    = 5.6e-3;%2.8e-3;                  % Indutância de filtro (H)
C    = 4.7e-6;%22e-6;                   % Capacitância de filtro (F)
Ro   = 22;                              % Resistência de carga (Ohm)
D    = 0.5;                             % Razão-cíclica
fs   = 10e3;                            % Frequência de chaveamento (Hz)
Ts   = 1/fs;                            % Período de chaveamento (s)
fclk = 72e6; %16e6; %64e6;                     % Frequência de clock (Hz)
...
fai = 2*fs;                               % Frequência de amostragem (Hz)
...
%----------------------%
wp_pii = 0;                              % Polo (rad/s)
wz_pii = wci/10;                         % Zero (rad/s)
Cpii   = tf([1 wz_pii],[1 wp_pii]);       % Função de transferência

k = 3530.9;                             % Ganho proporcional (sisotool)
Cpii2 = k * Cpii;                       % Adição do ganho proporcinal

ftmf = Cpii2*GiLdMa/(1+(Cpii2*GiLdMa))
...
%----------------------%
wp_piv = 0;                              % Polo (rad/s)
wz_piv = wcv/10;                         % Zero (rad/s);
Cpiv   = tf([1 wz_piv],[1 wp_piv]);       % Função de transferência

% OBS: Como a margem de fase ficou em 156º, não foi necessária a
%      inclusão de um compensador PD.

k = 0.044684;                            % Ganho proporcional (sisotool)
